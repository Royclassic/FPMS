                    ...
                    <div class="card-body card-padding">
                        <table  class="table table-bordered table-hover toggle-circle default footable-loaded footable" id="example" width="100%">
                            ...
                            @foreach($students as $key=>$student)
                                @if($student->documentation!==null)
                                    @if($student->documentation->final_documentation!==null && $student->documentation->status==1 )
                                        <tr>
                                            <td>{{$student->user->name}}</td>
                                            <td>{{$student->user->admission_staff_no}}</td>
                                            <td>{{$student->documentation->final_documentation}}</td>
                                            <td>{{$student->supervisor->user->name}}</td>
                                            <td>
                                                <a href="{{URL::to('documentations/'.$student->user->email.'/'.$student->documentation->final_documentation)}}" target="_blank">
