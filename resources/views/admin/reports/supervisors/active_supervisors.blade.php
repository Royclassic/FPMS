...
<h2 style="text-align: center">Active Supervisors</h2>
...
<table class="table table-bordered table-hover toggle-circle default footable-loaded footable">
    ...
    @foreach($supervisors as $key=>$supervisor)
        @if($supervisor->id!==1)
        @if($supervisor->passrec==1)
        <tr>
            <td>{{++$key}}</td>
            <td>{{$supervisor->name}}</td>
            <td>{{$supervisor->admission_staff_no}}</td>
            <td>{{$supervisor->faculty}}
            <td>{{$supervisor->email}}</td>
            <td>{{Carbon\Carbon::parse($supervisor->created_at)->format('d-m-Y')}}</td>
