...
<head>
    ...
    <title>Student Panel | {{ $pageTitle }}</title>
    <!-- Bootstrap Core CSS -->
    ...
    <nav class="navbar navbar-default navbar-static-top m-b-0">
        <div class="navbar-header">
            ...
            <div class="top-left-part">
                <!-- Logo -->
                <a class="logo hidden-xs hidden-sm" href="{{ route('student.dashboard.index') }}">
                    <!-- Logo icon image, you can use font-icon also -->
                @if(is_null($global->logo))
                    <!--This is dark logo icon-->
                        <img src="{{ asset('student.png') }}" alt="home" class="dark-logo client-logo"/>
                    @else
...
<script src="{{URL::to('bootstrap-select/dist/js/bootstrap-select.js')}}"></script>
<script>
    ...
    function addOrEditStickyNote(id) {
        ...
        if (id === undefined || id == "" || id == null) {
            url = '{{ route('student.sticky-note.store') }}'
        } else {

            url = "{{ route('student.sticky-note.update',':id') }}";
    ...
    function showCreateNoteModal() {
        var url = '{{ route('student.sticky-note.create') }}';
    ...
    function showEditNoteModal(id) {
        var url = '{{ route('student.sticky-note.edit',':id') }}';
        url = url.replace(':id', id);
    ...
    function deleteSticky(id) {
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                var url = "{{ route('student.sticky-note.destroy',':id') }}";
                url = url.replace(':id', id);
    ...
    function getNoteData() {
        ...
        var url = "{{ route('student.sticky-note.index') }}";
