                                       ...
                                       onclick="getChatData('{{$users->id}}', '{{$users->name}}')">
                                        @if(is_null($users->image))
                                            <img src="{{ asset('default-profile-2.png') }}" alt="user-img"
                                                 class="img-circle">
                                        @else
                                            <img src="{{ asset('profile/'.$users->image) }}" alt="user-img"
                                                 class="img-circle" height="30" width="30">
                                        @endif
                                        <span @if($users->message_seen == 'no' && $users->user_one != $user->id) class="font-bold" @endif> {{$users->name}}
                                            <small class="text-simple"> @if($users->last_message){{  \Carbon\Carbon::parse($users->last_message)->diffForHumans()}} @endif</small>
                                        </span>
                                    </a>
                                </li>
                    ...
                    <div class="chat-box ">
                        ...
                        <div class="row send-chat-box">
                            <div class="col-sm-12">
                                ...
                                <input type="text" name="message" id="submitTexts" placeholder="@lang("modules.messages.typeMessage")"
                                       class="form-control">
...
<script>
    ...
    $('#submitBtn').on('click', function (e) {
        ...
        if (submitText == "" || submitText == undefined || submitText == null) {
            ...
            var url = "{{ route('supervisor.user-chat.message-submit') }}";
    ...
    $("#userSearch").keyup(function (e) {
        var url = "{{ route('supervisor.user-chat.user-search') }}";
    ...
    function getChatData(id, dpName, scroll) {
        ...
        var url = "{{ route('supervisor.user-chat.index') }}";
    ...
    $('#new-chat').click(function () {
        var url = '{{ route('supervisor.user-chat.create')}}';
